(function () {
  ...
  function startLotus(containerId = "lotus-container") {
    const container = document.getElementById(containerId);
    if (!container) return;

    const lotuses = Array.from(container.querySelectorAll(".lotus"));
    lotuses.forEach((lotus) => lotus.classList.remove("visible"));

    let index = 0;
    if (window.lotusIntervals === undefined) window.lotusIntervals = {};
    if (window.lotusIntervals[containerId])
      clearInterval(window.lotusIntervals[containerId]);

    window.lotusIntervals[containerId] = setInterval(() => {
      if (index >= lotuses.length) {
        clearInterval(window.lotusIntervals[containerId]);
        delete window.lotusIntervals[containerId];
        return;
      }
      lotuses[index].classList.add("visible");
      index++;
    }, 400);
  }

  function stopLotus(containerId = "lotus-container") {
    const container = document.getElementById(containerId);
    if (!container) return;

    if (window.lotusIntervals && window.lotusIntervals[containerId]) {
      clearInterval(window.lotusIntervals[containerId]);
      delete window.lotusIntervals[containerId];
    }
...
function transitionToCelebrationPage() {
  ...
  startLotus("celebration-lotus-container");
}
